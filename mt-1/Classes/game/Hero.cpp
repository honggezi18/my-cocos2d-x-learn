...
{
	//根据向下行走动画的第一帧创建精灵
	auto a = sAnimationMgr->getAnimation(kDown)->getFrames().at(0)->getSpriteFrame();
	heroSprite = CCSprite::createWithSpriteFrame(a);
	//设置锚点
...
void Hero::fight()
{
	...
	Point pos = Vec2((targetPosition.x - getPosition().x) / 2 + 16, (targetPosition.y - getPosition().y) / 2 + 16);
